...
$fecc = date("d") + 5;
$mes = date("m")+1;
$feccc = date("Y-0").$mes.date("-d");
if ($fecc < 10) {
  $fechaC = date("Y-m-0") . $fecc;
} else {
...
<div class="row">
  <div class="col-md-12">
    <a style=" margin-left: 10px;" title="Registrar Nuevo" class="btn red btn-outline sbold " data-toggle="modal" href="#productoguarda">Nuevo ingreso de Materia prima </a>

    <a style=" margin-left: 10px;" class="btn red btn-outline sbold " title="Actualizar tabla" data-toggle="modal" href="?admin=entradaM"> <i class="fa fa-refresh" aria-hidden="true"></i> </a>

    <div class="portlet light ">
      <div class="portlet-title">
        <div class="caption font-dark">
          <i class="icon-settings font-dark"></i>
          <span class="caption-subject bold uppercase">Registros de Materia Prima</span>
            ...
            while ($fila = $bd->mostrar_registros()) { ?>
              <?php if (($fila->tipo_producto) == "Materia Prima") { ?>
                ...
                <tr>
                  ...
                  <?php } else if (($fila->fecha_entrada) < $feccc ) { ?>
                    <?php if(($fila->fecha_venci) > $feccc){?>
                      <td width="20%" style="background-color:#C0FF52"><?php echo $fila->fecha_venci; ?>
                      <i class="fa fa-calendar-check-o" aria-hidden="true">Buen estado</i>
                    </td>
                    <?php } else {?>
                    <td width="20%" style="background-color:#FFFC52"><?php echo $fila->fecha_venci; ?>
                      <i class="fa fa-calendar-minus-o" aria-hidden="true">Proximo a caducar</i>
                    </td>
                  <?php }} ?>
                  <td width="25%" class="hidden-print noprin">
                    <center>
                      ...
                      <a class="btn red btn-outline sbold " title="Eliminar" onclick='if(confirmDel() == false){return false;}' class="btn red btn-outline sbold" href='?admin=entradaM&eliminar&codigo=<?php echo $id ?>'><i class=' fa fa-trash'></i></a>
              ...
              <div class="table-scrollable">
                <table class="table table-striped table-hover">
                  <thead>
                    <tr>
                      <div id="register_form">
                        <form role="form" action="?admin=entradaM&crear=crear" method="post" enctype="multipart/form-data">
                          <th>Materia Prima <> Descripcion </th>
                          ...
                          while ($fila = $bd->mostrar_registros()) { ?>
                            <?php if (($fila->tipo_producto) == "Materia Prima") { ?>
                              <option value="<?php echo $id = $fila->id_service; ?>">
                                <p><?php echo $id = $fila->name_service . " <> " . $fila->info_service; ?></p>
                              </option>
                          <?php
                            }
                          }
                  ...
                  <tbody>
                    <tr>
                      <td width="30%">
                        * <input required type="number" required name="cantida" required class="form-control" min="1" max="100">
                      </td>
                      <td width="20%">
